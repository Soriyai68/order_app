<template>
  <section class="container px-2 mx-auto">
    <div class="flex items-center gap-x-3">
      <h2 class="text-lg font-medium text-gray-800 dark:text-white">Brands</h2>
      <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">{{ brands.length }} brands</span>
    </div>
    <!-- Alert -->
    ...
    <!-- Open Modal Button -->
    <div class="my-6 flex">
      <button @click="openModal" class="px-6 py-2 mx-auto tracking-wide text-white capitalize transition-colors duration-300 transform bg-blue-600 rounded-md hover:bg-blue-500 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-80">
        Add Brand
      </button>
    </div>
    <!-- Modal -->
    <div v-if="modalOpen" class="fixed inset-0 z-10 flex items-center justify-center overflow-y-auto bg-black bg-opacity-40" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div class="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-middle transition-all transform bg-white rounded-lg shadow-xl dark:bg-gray-900 sm:my-8 sm:w-full sm:max-w-sm sm:p-6 sm:align-middle">
        <h3 class="text-lg font-medium leading-6 text-gray-800 capitalize dark:text-white" id="modal-title">
          {{ form._id ? 'Edit Brand' : 'Add Brand' }}
        </h3>
        <form class="mt-4" @submit.prevent="submit">
          <div class="mb-3">
            <label class="text-sm text-gray-700 dark:text-gray-200" for="brand-name">Name</label>
            <input id="brand-name" v-model="form.name" required placeholder="Brand name"
            ...
            />
            <span v-if="nameExists" class="text-xs text-red-500">Name already exists.</span>
          </div>
          <div class="mb-3">
            <label class="text-sm text-gray-700 dark:text-gray-200" for="brand-description">Description</label>
            <input id="brand-description" v-model="form.description" placeholder="Description"
              class="block w-full mt-1 px-4 py-3 text-sm text-gray-700 bg-white border border-gray-200 rounded-md focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300 dark:focus:border-blue-300" />
          </div>
          <div class="mt-4 sm:flex sm:items-center sm:-mx-2">
            <button type="button" @click="closeModal" class="w-full px-4 py-2 text-sm font-medium tracking-wide text-gray-700 capitalize transition-colors duration-300 transform border border-gray-200 rounded-md sm:w-1/2 sm:mx-2 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800 hover:bg-gray-100 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-40">
              Cancel
            </button>
    ...
    <div v-if="deleteModal.show" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-sm w-full">
        <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">Delete Brand</h3>
        <p class="text-gray-600 dark:text-gray-300 mb-4">Are you sure you want to delete this brand?</p>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                  ...
                  <th class="px-12 py-3.5 text-sm font-normal text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                <tr v-for="brand in pagedBrands" :key="brand._id">
                  <td class="px-4 py-4 text-sm font-medium text-blue-500 whitespace-nowrap">
                    {{ brand.name }}
                  </td>
                  ...
                  <td class="px-4 py-4 text-sm whitespace-nowrap">
                    <div class="flex items-center gap-x-6">
                      <button @click="edit(brand)" class="text-gray-500 transition-colors duration-200 dark:hover:text-yellow-500 dark:text-gray-300 hover:text-yellow-500 focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                        </svg>
                      </button>
                      <button @click="askDelete(brand._id)" class="text-gray-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-300 hover:text-red-500 focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                ...
                <tr v-if="brands.length === 0">
                  <td colspan="3" class="px-4 py-4 text-center text-gray-500">No brands found.</td>
                </tr>
              </tbody>
            </table>
            <!-- Pagination Controls -->
...
export default {
  ...
  computed: {
    ...
    filteredBrands() {
      ...
      return this.brands.filter(b => b.name && b.name.toLowerCase().includes(search));
    }
  },
  methods: {
    ...
    submit() {
      // Check for duplicate name (case-insensitive, ignore self on edit)
      const nameTrim = (this.form.name || '').trim().toLowerCase();
      const exists = this.brands.some(
        b => b.name.trim().toLowerCase() === nameTrim && b._id !== this.form._id
      );
      this.nameExists = exists;
      if (exists) {
        this.showAlert('error', 'Error', 'Brand name already exists!');
        return;
      }
      if (this.form._id) {
        Meteor.call('brands.update', {
          _id: this.form._id,
          name: this.form.name,
          ...
          if (err) {
            ...
            this.fetchBrands()
            this.closeModal()
            this.showAlert('success', 'Success', 'Brand updated successfully!')
          }
        })
      } else {
        Meteor.call('brands.insert', {
          ...
          if (err) {
            ...
            this.fetchBrands()
            this.closeModal()
            this.showAlert('success', 'Success', 'Brand added successfully!')
          }
        })
      }
    },
    ...
    confirmDelete() {
      ...
      if (_id) {
        // Check if brand is used in any product
        Meteor.call('products.find', {}, (err, products) => {
          if (err) {
            this.showAlert('error', 'Error', err.reason || err.message || err)
            return
          }
          if (products && products.some(p => p.brand_id === _id)) {
            this.showAlert('error', 'Error', 'Cannot delete: Brand is used in products.')
            return
          }
          Meteor.call('brands.remove', { _id }, (err) => {
            if (err) {
              this.showAlert('error', 'Error', err.reason || err.message || err)
            } else {
              this.fetchBrands()
              this.showAlert('success', 'Success', 'Brand deleted successfully!')
            }
          })
        })
      }
