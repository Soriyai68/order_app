<template>
  <section class="container px-2 mx-auto">
    <div class="flex items-center gap-x-3">
      <h2 class="text-lg font-medium text-gray-800 dark:text-white">Units</h2>
      <span
      ...
      >
        ...
        <form class="mt-4" @submit.prevent="submit">
          <div class="mb-3">
            ...
            />
            <span v-if="nameExists" class="text-xs text-red-500"
          ...
          <div class="mb-3">
            <label class="text-sm text-gray-700 dark:text-gray-200 block mb-1"
              >Type</label
            >
            <div class="flex gap-4">
              <label class="inline-flex items-center">
                <input type="radio" value="float" v-model="form.type" />
                <span class="ml-2">Float</span>
              </label>
              <label class="inline-flex items-center">
                <input type="radio" value="int" v-model="form.type" />
                <span class="ml-2">Int</span>
              </label>
            </div>
          </div>
        ...
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
          <div class="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-xs sm:text-sm md:text-base">
              ...
              >
                <tr v-for="unit in pagedUnits" :key="unit._id">
                  <td
                  ...
                  <td
                    class="px-12 py-4 text-sm font-medium text-emerald-500 whitespace-nowrap"
                  >
                    {{ unit.sub_name }}
                  </td>
                  <td
...
export default {
  ...
  computed: {
    ...
    filteredUnits() {
      if (!this.searchName.trim()) return this.units;
      const search = this.searchName.trim().toLowerCase();
      return this.units.filter(u => u.name && u.name.toLowerCase().includes(search));
    },
  },
  methods: {
    ...
    submit() {
      ...
      const exists = this.units.some(
        (u) =>
          u.name.trim().toLowerCase() === nameTrim && u._id !== this.form._id
      );
      this.nameExists = exists;
      if (exists) {
        this.showAlert("error", "Error", "Unit name already exists!");
        return;
      }
      const payload = {
        name: this.form.name,
      ...
      if (this.form._id) {
        ...
        Meteor.call("units.insert", payload, (err) => {
          if (err) {
            this.showAlert("error", "Error", err.reason || err.message || err);
          } else {
            this.fetchUnits();
            this.closeModal();
            this.showAlert("success", "Success", "Unit added successfully!");
          }
        });
      }
    },
    ...
    confirmDelete() {
      ...
      if (_id) {
        ...
        Meteor.call("products.find", {}, (err, products) => {
          ...
          if (products && products.some(p => p.unit_id === _id)) {
            this.showAlert("error", "Error", "Cannot delete: Unit is used in products.");
            return;
          }
          Meteor.call("units.remove", { _id }, (err) => {
            if (err) {
              this.showAlert("error", "Error", err.reason || err.message || err);
            } else {
              this.fetchUnits();
              this.showAlert("success", "Success", "Unit deleted successfully!");
            }
          });
        });
      }
    ...
    onIntInput(e) {
      ...
      if (e && e.target && e.target.value) {
        ...
        if (val.includes(".")) {
          ...
          this.form.int = e.target.value ? parseInt(e.target.value, 10) : null;
        }
      }
    },
  },
