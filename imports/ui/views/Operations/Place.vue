<template>
  <section class="container px-2 mx-auto">
    <div class="flex items-center gap-x-3">
      <h2 class="text-lg font-medium text-gray-800 dark:text-white">Place Orders</h2>
      <span class="px-3 py-0 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">
        {{ orders.length }} orders
      </span>
    </div> <!-- Add Order Button -->
    <div class="my-6 flex">
    ...
    <div v-if="alert.show"
      ...
      <div :class="['flex items-center justify-center w-12', alert.bg]">
        <svg v-if="alert.type === 'success'" class="w-6 h-6 text-white fill-current" viewBox="0 0 40 40">
    ...
    <div v-if="modalOpen"
      ...
      <div
        ...
        <h3 class="text-lg font-medium leading-6 text-gray-800 capitalize dark:text-white" id="modal-title">
          {{ form._id ? "Edit Order" : "Add Order" }}
        </h3>
        <form class="mt-4" @submit.prevent="submit">
          <div class="mb-3">
            <label class="text-sm text-gray-700 dark:text-gray-200" for="order-name">Name</label>
            <input id="order-name" v-model="form.name" required placeholder="Order name"
          ...
          <div class="mb-3">
            <label class="text-sm text-gray-700 dark:text-gray-200" for="order-date">Date</label>
            <input id="order-date" type="date" v-model="form.dateStr" required
              class="block w-full mt-1 px-4 py-3 text-sm text-gray-700 bg-white border border-gray-200 rounded-md focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300 dark:focus:border-blue-300" />
          </div>
          <div class="mb-3">
            <label class="text-sm text-gray-700 dark:text-gray-200" for="order-description">Description</label>
            <input id="order-description" v-model="form.description" placeholder="Description"
              class="block w-full mt-1 px-4 py-3 text-sm text-gray-700 bg-white border border-gray-200 rounded-md focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300 dark:focus:border-blue-300" />
          </div>
    ...
    <div v-if="deleteModal.show"
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-sm w-full">
        <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">Delete Order</h3>
        <p class="text-gray-600 dark:text-gray-300 mb-4">Are you sure you want to delete this order?</p>
          ...
          <div class="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-xs sm:text-sm md:text-base">
              <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                  <th class="py-3.5 px-4 text-sm font-normal text-left text-gray-500 dark:text-gray-400">Name</th>
              ...
              <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                <tr v-for="order in pagedOrders" :key="order._id">
                  <td class="px-4 py-4 text-sm font-medium text-blue-500 whitespace-nowrap">{{ order.name }}</td>
                  <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">
                    {{ formatDate(order.date) }}
                  </td>
                  <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{ order.description }}</td>
                  <td class="px-4 py-4 text-sm whitespace-nowrap">
                    <div class="flex items-center gap-x-6">
                      ...
                      <button @click="askDelete(order._id)"
...
export default {
  ...
  computed: {
    filteredOrders() {
      if (!this.searchName.trim()) return this.orders;
      const search = this.searchName.trim().toLowerCase();
      return this.orders.filter(
        (o) => o.name && o.name.toLowerCase().includes(search)
      );
    },
    totalPages() {
  ...
  methods: {
    ...
    submit() {
      if (!this.form.name || !this.form.dateStr) {
        this.showAlert("error", "Error", "Name and date are required!");
        return;
      }
      const payload = {
        name: this.form.name,
      ...
      if (this.form._id) {
        Meteor.call("placeOrder.update", { _id: this.form._id, ...payload }, (err) => {
          if (err) {
        ...
        Meteor.call("placeOrder.insert", payload, (err) => {
          if (err) {
            this.showAlert("error", "Error", err.reason || err.message || err);
          } else {
            this.fetchOrders();
            this.closeModal();
            this.showAlert("success", "Success", "Order added successfully!");
          }
        });
      }
    },
    ...
    confirmDelete() {
      ...
      if (_id) {
        Meteor.call("placeOrder.remove", { _id }, (err) => {
          if (err) {
            this.showAlert("error", "Error", err.reason || err.message || err);
          } else {
            this.fetchOrders();
    ...
    edit(order) {
      this.form = {
        ...
        name: order.name,
        date: order.date,
        dateStr: order.date ? this.toDateInput(order.date) : "",
    ...
    openModal() {
      this.form = {
        ...
        name: "",
        date: "",
        dateStr: this.toDateInput(new Date()),
        description: "",
      };
      this.modalOpen = true;
    },
